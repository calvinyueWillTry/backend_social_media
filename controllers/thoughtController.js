...
module.exports = {
  ...
  async getCourses(req, res) {
    try {//query all courses from the database.populate the students field (including details) for each course
      const courses = await Thought.find().populate('students');
...
//delete all students associated with that course
      await User.deleteMany({ _id: { $in: course.students } });//a filter that matches the _id of the students that are in the course.students array.
  ...
  async updateCourse(req, res) {
    try {//find and update a course by its _id
      const course = await Thought.findOneAndUpdate(//It takes three parameters:
